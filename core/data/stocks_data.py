...
_global_fetcher: Optional[StocksFetcher] = None


def _get_global_fetcher() -> StocksFetcher:
    """Get or create global stocks fetcher instance."""
    global _global_fetcher
    if _global_fetcher is None:
        _global_fetcher = StocksFetcher(cache_ttl=60)
    return _global_fetcher


async def fetch_stock_quotes() -> List[Dict[str, Any]]:
    """
    Fetch current stock quotes (backward compatible).
...
def get_market_status() -> str:
    """
    Determine if market is open or closed (backward compatible).
    
    Returns:
        "open", "closed", or "pre-market"
    """
    return StocksFetcher.get_market_status()


async def fetch_market_gainers(limit: int = 10) -> List[Dict[str, Any]]:
    """
    Fetch top gaining stocks (backward compatible).
    ...
    """
    fetcher = _get_global_fetcher()
    return await fetcher.fetch_screener(ScreenerType.GAINERS, limit)


async def fetch_market_losers(limit: int = 10) -> List[Dict[str, Any]]:
    """
    Fetch top losing stocks (backward compatible).
    ...
    """
    fetcher = _get_global_fetcher()
    return await fetcher.fetch_screener(ScreenerType.LOSERS, limit)


async def fetch_market_active(limit: int = 10) -> List[Dict[str, Any]]:
    """
    Fetch most actively traded stocks (backward compatible).
    ...
    """
    fetcher = _get_global_fetcher()
    return await fetcher.fetch_screener(ScreenerType.MOST_ACTIVE, limit)


async def fetch_market_mixed(limit: int = 10) -> List[Dict[str, Any]]:
    """
    Fetch a mix of top gainers and losers (backward compatible).
    ...
    """
    fetcher = _get_global_fetcher()
    return await fetcher.fetch_mixed_screener(limit)
