...
_global_fetcher: Optional[SportsFetcher] = None


def _get_global_fetcher() -> SportsFetcher:
    """Get or create global sports fetcher instance."""
    global _global_fetcher
    if _global_fetcher is None:
        _global_fetcher = SportsFetcher(cache_ttl=60)
    return _global_fetcher


async def fetch_all_games() -> List[Dict[str, Any]]:
    """
    Fetch all games for your configured teams (backward compatible).
    
    Returns:
        List of game dicts (all states)
    """
    fetcher = _get_global_fetcher()
    return await fetcher.fetch_games(filter_teams=True)


async def fetch_upcoming_games(today_only: bool = False) -> List[Dict[str, Any]]:
    """
    Fetch upcoming games (backward compatible).
    
    Args:
        today_only: If True, only return games scheduled for today
    ...
    if today_only:
        # Filter by date (same logic as before)
        ...
        for game in games:
            ...
            try:
                ...
                filtered.append(game)  # Include if parsing fails
        
        games = filtered
    
    logger.info(f"Found {len(games)} upcoming games" + (" today" if today_only else ""))
    return games


async def fetch_all_live_games() -> List[Dict[str, Any]]:
    """
    ...
    """
    fetcher = _get_global_fetcher()
    return await fetcher.fetch_games(
        states=[GameState.LIVE],
        filter_teams=False
    )


async def fetch_all_upcoming_games() -> List[Dict[str, Any]]:
    """
...
async def fetch_live_games_by_leagues(leagues: List[str]) -> List[Dict[str, Any]]:
    """
